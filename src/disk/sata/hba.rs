...
impl HbaPort {
    ...
    pub fn init(&mut self, clb: &mut Dma<[HbaCmdHeader; 32]>, ctbas: &mut [Dma<HbaCmdTable>; 32], fb: &mut Dma<[u8; 256]>) {
        use screen::{fill, WRITER, Color, ColorCode};
        // debug!("Stop");
        self.stop();
        ...
        if let Some(slot) = self.slot() {
            {
                ...
                // unsafe { ptr::write_bytes(cmdtbl.deref_mut() as *mut HbaCmdTable as *mut u8, 0, 0x100080); } // 0x100080 = size_of::<HbaCmdTable>()
    ...
    pub fn ata_stop(&mut self, slot: u32) -> Result<()> {
        ...
        if self.is.read() & HBA_PORT_IS_ERR != 0 {
            // println!("{}", format!("ERROR IS {:X} IE {:X} CMD {:X} TFD {:X}\nSSTS {:X} SCTL {:X} SERR {:X} SACT {:X}\nCI {:X} SNTF {:X} FBS {:X}\n",
...
impl HbaMem {
    pub fn init(&mut self) {
        ...
        */
        ...
        // println!("{}", format!("   - AHCI CAP {:X} GHC {:X} IS {:X} PI {:X} VS {:X} CAP2 {:X} BOHC {:X}",
