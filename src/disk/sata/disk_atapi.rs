...
impl DiskATAPI {
    pub fn new(id: usize, port: &'static mut HbaPort) -> Result<Self> {
        use crate::screen::{fill, WRITER, ColorCode, Color};
...
impl Disk for DiskATAPI {
    ...
    fn read(&mut self, block: u64, buffer: &mut [u8]) -> Result<Option<usize>> {
        ...
        if blk_len != 0 {
            sectors = buffer.len() as u32 / blk_len;
        }
        else {
            sectors = buffer.len() as u32 / 512;
        }
        ...
        let mut buf_size = 0;
        if blk_len != 0 {
            buf_size = buf_len * blk_len
        }
        else {
            buf_size = buf_len * 512;
        }
